...
### Solution
Jun 26, 2026, [Time Taken 11m 48s]
```python
class Solution:
    def maxOperations(self, nums: List[int], k: int) -> int:
        # S: 1. Save every nums[i] into hashmap
        # 2. Loop over nums again, if k - nums[i] in hashmap, update res and decrement the diff in hashmap
        ...
        # 2. Calculate diff in hashmap
        for n in nums:
            diff = k - n
            # Need to make sure both diff and n exists in hashmap
            if diff in hashmap and n in hashmap:
                if diff != n:
                    res += 1
                    hashmap[n] -= 1
                    hashmap[diff] -= 1
                    ...
                    # If diff = n, we need to ensure we have hashmap[diff] >= 2
                    if hashmap[n] >= 2:
                        res += 1
                        hashmap[n] -= 1
                        hashmap[n] -= 1
        ...
        return res
```
**Time Complexity:** $O(n)$, `n` is the length of `nums`, because sorting costs $O(nlog\ n)$. <br>
**Space Complexity:** $O(n)$

<br>

### In-place solution
Sort first and two ptrs
```python
class Solution:
    def maxOperations(self, nums: List[int], k: int) -> int:
