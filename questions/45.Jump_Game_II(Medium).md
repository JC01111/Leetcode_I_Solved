...
### Solution
Jul 17, 2026. [Time Taken 17m 29s]
```python
class Solution:
    def jump(self, nums: List[int]) -> int:
        # Q: Find min steps to jump to reach the end
        # S: Use dp[i] = min jumps to reach
        # Intialize all inf for each index i in dp[]
        # For each index i, we check the range it can reach from i, i + nums[i]
        # dp[i] = min(dp[i], dp[j] + dp[i]), either take original jumps or a new smaller jumps for each
        # TC: O(n!), SC: O(n)
        dp = [0] + [inf] * (len(nums)-1)
