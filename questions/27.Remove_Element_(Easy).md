...
class Solution:
    def removeElement(self, nums: List[int], val: int) -> int:
        # S: Maintain two ptrs to keep track of val and non-val element
        # When val is found in nums, we keep the val ptr there until non-val ptr has non-val element, then sawp
        # TC: O(n), n=len(nums), SC: O(1)
        i = 0
        for j in range(len(nums)):
            # Swap or to move together
            if nums[j] != val:
                nums[j], nums[i] = nums[i], nums[j]
                i += 1
            # Otherwise, we don't move until nums[j] == non-val
        return i
```
