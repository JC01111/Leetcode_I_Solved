...
2. Use sliding window and handle when `nums[r] != nums[r-1] + 1`, update `res[]` base on `nums[l] == nums[r-1]` or not.
...
### Solution
Jul 14, 2026. [Time Taken 14m 12s]
```python
class Solution:
    def summaryRanges(self, nums: List[int]) -> List[str]:
        # Q: Return summary of ranges
        # S: Sliding window to summarize nums
        # We maintain consecutive/ascending range with two ptrs, 
        # Handle the last case after loop
        ...
        while r < len(nums):
            ...
            if r > 0 and nums[r] != (nums[r-1] + 1):
                if nums[l] == nums[r-1]:
                    ...
                    string = str(nums[l]) + "->" + str(nums[r-1])
                    res.append(string)
                # Update l
                l = r
            r += 1
        # Handle the last case, check if nums[l] == nums[r]
        if nums[l] == nums[r-1]:
            res.append(str(nums[r-1]))
        else:
            string = str(nums[l]) + "->" + str(nums[r-1])
