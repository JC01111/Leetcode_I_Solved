...
### Deque
Jul 8, 2026. [Time Taken 20m 23s]

This is not optimized, stack is enough to handle.
```python
class Solution:
    def dailyTemperatures(self, temperatures: List[int]) -> List[int]:
        # S: maintain a minHeap [temp, index], while new temperature > minHeap[-1], update indices correspondly
        # TC: O(nlogn), n=len(temperatures), SC: O(n)
        res = [0] * len(temperatures)
        minHeap = []     # [[temp, index]]
        # Start processing
        for i, temp in enumerate(temperatures):
            # Update previous temperatures' indices
            while minHeap and temp > minHeap[0][0]:
                _, oldIndex = heapq.heappop(minHeap)
