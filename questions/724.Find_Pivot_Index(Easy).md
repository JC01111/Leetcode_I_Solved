...
### Solution
Jun 29, 2026, [Time Taken 8m 13s]
```python
class Solution:
    def pivotIndex(self, nums: List[int]) -> int:
        # Q: Return the index which leftSum == rightSum
        # S: 1. Calculate the prefixSum
        # Each index, we compare if prefixSum == total - num - prefixSum, if so, return True
        # TC: O(n), SC: O(1)
        total = sum(nums)
        prefix = 0
