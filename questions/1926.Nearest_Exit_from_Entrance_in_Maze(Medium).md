...
### Solution
Jul 15, 2026. [Time Taken 14m 55s]
```python
class Solution:
    def nearestExit(self, maze: List[List[str]], entrance: List[int]) -> int:
        # S: 1. Run BFS from entrance to the nearest exit (boundaries)
        # 2. Update res only reach boundary
        # TC: O(m*n), SC: O(m*n)
        ...
        while deque:
            ...
            for dr, dc in directions:
                newR, newC = r+dr, c+dc
                if newR in range(len(maze)) and newC in range((len(maze[0]))) and (newR, newC) not in visited and maze[newR][newC] == '.':
                    # Check if new pos is an exit
                    if newR == 0 or newR == len(maze)-1 or newC == 0 or newC == len(maze[0])-1:
                        res = min(res, steps+1)
                    else:
