...
### Solution
Jul 13, 2026. [Time Taken 12m 14s]
```python
...
class Solution:
    def maxLevelSum(self, root: Optional[TreeNode]) -> int:
        # Q: Find smallest level x of maximal sum of nodes at the same level
        # S: 1. Run BFS to calculate the sum at each level. 2. Update res (index) only if newSum > curSum
        # TC: O(n), n is total nodes, SC: O(n)
        deque = collections.deque([[root, 1]])
        res, curSum = 1, root.val
        # Run BFS
        while deque:
            curr = 0
