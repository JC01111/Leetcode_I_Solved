...
### Solution
Jul 12, 2026. [Time Taken 4m 15s]
```python
class Solution:
    def eraseOverlapIntervals(self, intervals: List[List[int]]) -> int:
        # S: 1. Use stack to save intervals
        # 2. If new start_j < end_i, update stack[-1] to keep the min, update res += 1
        ...
        for start_j, end_j in intervals:
            # Case with overlap
            if stack and stack[-1][1] > start_j:
                res += 1
                stack[-1][1] = min(stack[-1][1], end_j)
            else:
                stack.append([start_j, end_j])
        return res
```
