...
class Solution:
    def longestZigZag(self, root: Optional[TreeNode]) -> int:
        # S: start traversing from each node with left/right
        # Select the most at each node and update res
        # TC: O(n), SC: O(n)
        # 1. Run DFS on root and find max path for each node
        res = 0
        def dfs(node, direct, depth):
            nonlocal res
            if not node:
                return depth
            res = max(res, depth)
            # For each direction, consider if we can go or start a new direction
            if direct == 'left':
